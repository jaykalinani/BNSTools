...
//     {"/rho_potential",rho_potential,"Values for RNSID variable
//     rho_potential"},
...
//     status = H5Awrite(attribute_id,attributeH5type,
//     attrblist[varIndex].data); status = H5Aclose(attribute_id); status =
//     H5Sclose(dataspace_id);
// #ifdef RNS_SEQ_COMPILATION
//     if (attrblist[varIndex].H5type == H5T_NATIVE_DOUBLE) {
//       printf("Save attribute %s value is
//       %5.4e\n",attrblist[varIndex].name,*((double
//       *)attrblist[varIndex].data));
...
//     } else {
//       printf("Save attribute %s value is not
...
//   H5P_DEFAULT, 		      s_qp); status = H5Sclose(dataspace_id); status =
//   H5Dclose(dataset_id); dims[0]= (*mdiv)+1; dataspace_id =
//   H5Screate_simple(1,dims, NULL); dataset_id = H5Dcreate(file_id,"/mu",
//                          H5T_NATIVE_DOUBLE, dataspace_id,
//   			 H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
//   status = H5Dwrite(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL,
//   H5P_DEFAULT, 		      mu); status = H5Sclose(dataspace_id); status =
//   H5Dclose(dataset_id);
//
//   /* ============================================= */
...
// 			      dataspace_id,  H5P_DEFAULT, H5P_DEFAULT);
//     status = H5Awrite(attribute_id,attributeH5c128,
//     varlist[varIndex].description); status = H5Aclose(attribute_id); status =
//     H5Dclose(dataset_id);
//
//
//   }
...
void hdf5_read_var(int *sdiv, int *mdiv, const char fname[],
                   /* ------------------- */
                   char eos_type[], char eos_file[], double *eos_k,
                   double *Gamma_P,
                   /* ------------------- */
                   char rotation_type[], double *A_diff,
                   /* ------------------- */
                   double *r_ratio, double *rho0_center, double *r_e,
                   /* ------------------- */
                   double *s_qp, /*SDIV+1*/
                   double *mu,   /*MDIV+1*/
                                 /* ------------------- */
                   ...
                   double *Omega_e, double **Omega_diff) {
  ...
  struct {
    ...
    hid_t H5type;
    void *data;
  } attrblist[] = {{"poly K", H5T_NATIVE_DOUBLE, eos_k},
                   {"poly Gamma", H5T_NATIVE_DOUBLE, Gamma_P},
                   {"axis_ratio", H5T_NATIVE_DOUBLE, r_ratio},
                   {"re", H5T_NATIVE_DOUBLE, r_e},
                   {"rhoc", H5T_NATIVE_DOUBLE, rho0_center},
  ...
  struct {
    ...
    char description[128];
      ...
      {"/rho_potential", rho_potential,
       "Values for RNSID variable rho_potential"},
      {"/gama", gama, "Values for RNSID variable gama         "},
      {"/alpha", alpha, "Values for RNSID variable alpha        "},
      {"/omega", omega, "Values for RNSID variable omega        "},
      {"/energy", energy, "Values for RNSID variable energy       "},
  ...
  for (varIndex = 0; varIndex < attrblistLEN; varIndex++) {
    if (attrblist[varIndex].H5type == H5T_C_S1) {
      attributeH5type = H5Tcopy(H5T_C_S1);
      H5Tset_size(attributeH5type,
                  strlen((const char *)attrblist[varIndex].data) + 1);
      dims[0] = 1;
    } else {
      dims[0] = 1;
      attributeH5type = attrblist[varIndex].H5type;
    ...
    if (attrblist[varIndex].H5type == H5T_NATIVE_DOUBLE)
      printf("Read attribute %s value is %5.4e\n", attrblist[varIndex].name,
             *((double *)attrblist[varIndex].data));
    else if (attrblist[varIndex].H5type == H5T_NATIVE_INT)
      printf("Read attribute %s value is %d\n", attrblist[varIndex].name,
             *((int *)attrblist[varIndex].data));
    else if (attrblist[varIndex].H5type == H5T_C_S1)
      printf("Read attribute %s value is %s\n", attrblist[varIndex].name,
             (char *)attrblist[varIndex].data);
    else
