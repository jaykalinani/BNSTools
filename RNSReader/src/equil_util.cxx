...
/*************************************************************************/
double interp(double xp[], double yp[], int np, double xb, int *n_nearest_pt) {
...
double deriv_ss(double **f, int s, int m) {
  ...
  switch (s) {
    ...
    */
    /*
               case 1    :
       d_temp=(f[s][m]-2.0*f[s+1][m]+f[s+2][m])/(2.0*SQ(DS)); break;

               case 2    :
       d_temp=(2.0*f[s-1][m]-3.0*f[s][m]+f[s+2][m])/(4.0*SQ(DS)); break;
       ...
       d_temp=(f[s][m]-2.0*f[s-1][m]+f[s-2][m])/(2.0*SQ(DS)); break;
               ...
               default     :
       d_temp=(f[s+2][m]-2.0*f[s][m]+f[s-2][m])/(4.0*SQ(DS)); break;
    */
...
double deriv_sm(double **f, int s, int m) {
  ...
  case 1:
    if (m == 1) {
      d_temp =
          (f[s + 1][m + 1] - f[s][m + 1] - f[s + 1][m] + f[s][m]) / (DM * DS);
    } else {
      if (m == MDIV) {
        d_temp =
            (f[s + 1][m] - f[s][m] - f[s + 1][m - 1] + f[s][m - 1]) / (DM * DS);
      } else {
        d_temp =
            (f[s + 1][m + 1] - f[s + 1][m - 1] - f[s][m + 1] + f[s][m - 1]) /
            (2.0 * DM * DS);
      }
    }
    break;
  ...
  case SDIV:
    if (m == 1) {
      d_temp =
          (f[s][m + 1] - f[s][m] - f[s - 1][m + 1] + f[s - 1][m]) / (DM * DS);
    } else {
      if (m == MDIV) {
        d_temp =
            (f[s][m] - f[s - 1][m] - f[s][m - 1] + f[s - 1][m - 1]) / (DM * DS);
      } else {
        d_temp =
            (f[s][m + 1] - f[s][m - 1] - f[s - 1][m + 1] + f[s - 1][m - 1]) /
            (2.0 * DM * DS);
      }
    }
    break;
  ...
  default:
    if (m == 1) {
      d_temp = (f[s + 1][m + 1] - f[s - 1][m + 1] - f[s + 1][m] + f[s - 1][m]) /
               (2.0 * DM * DS);
    } else {
      if (m == MDIV) {
        d_temp =
            (f[s + 1][m] - f[s - 1][m] - f[s + 1][m - 1] + f[s - 1][m - 1]) /
            (2.0 * DM * DS);
      } else {
        d_temp = (f[s + 1][m + 1] - f[s - 1][m + 1] - f[s + 1][m - 1] +
                  f[s - 1][m - 1]) /
                 (4.0 * DM * DS);
      }
    }
...
/* (based on rtsec) */
/*******************************************************************/
double rtsec_G(double (*func)(double, double, double), double Gamma_P,
               double x1, double x2, double xacc, double ee, double eos_k)
...
double zbrent_diff(double (*func)(double, double, double, double, double,
                   ...
                   double r_e, double rho_equator_h, double gama_equator_h,
                   double omega_equator_h, double rho_pole_h,
                   double gama_pole_h, double x1, double x2, double tol,
                   double A_diff) {
  int iter;

  double a = x1, b = x2, c = x2, d, e, min1, min2;

  double fa = (*func)(a, r_e, rho_equator_h, gama_equator_h, omega_equator_h,
                      rho_pole_h, gama_pole_h, A_diff),
         fb = (*func)(b, r_e, rho_equator_h, gama_equator_h, omega_equator_h,
                      rho_pole_h, gama_pole_h, A_diff),
         fc, p, q, r, s, tol1, xm;
...
/* (based on zbrent) */
/**************************************************************************/
double zbrent_rot(double (*func)(double, double, double, double, double, double,
                                 double, double),
                  double r_e, double rhogp, double omegagp, double sgp,
                  double mugp, double Omega_c, double x1, double x2, double tol,
                  double A_diff) {
  int iter;

  double a = x1, b = x2, c = x2, d, e, min1, min2;

  double fa = (*func)(a, r_e, rhogp, omegagp, sgp, mugp, Omega_c, A_diff),
         fb = (*func)(b, r_e, rhogp, omegagp, sgp, mugp, Omega_c, A_diff), fc,
         p, q, r, s, tol1, xm;
...
/**************************************************************************/
double diff_rotation(double x, double r_e, double rho_equator_h,
                     double gama_equator_h, double omega_equator_h,
                     double rho_pole_h, double gama_pole_h, double A_diff) {
  return (SQ(r_e) *
              (gama_equator_h + rho_equator_h - gama_pole_h - rho_pole_h) +
          log(1.0 -
              SQ((x - omega_equator_h) * exp(-SQ(r_e) * rho_equator_h)))) *
             SQ(1.0 -
                SQ((x - omega_equator_h) * exp(-SQ(r_e) * rho_equator_h))) -
         SQ((1.0 / A_diff) * (x - omega_equator_h) *
            exp(-2.0 * SQ(r_e) * rho_equator_h));
}

/**************************************************************************/
double rotation_law(double x, double r_e, double rhogp, double omegagp,
                    double sgp, double mugp, double Omega_c, double A_diff) {

  return (Omega_c - x) *
             (SQ(1.0 - sgp) - SQ(sgp * (x - omegagp) * exp(-SQ(r_e) * rhogp)) *
                                  (1.0 - mugp * mugp)) -
         SQ((1.0 / A_diff)) * (x - omegagp) * sgp * sgp * (1.0 - mugp * mugp) *
             exp(-2.0 * SQ(r_e) * rhogp);
}
} // namespace RNSReader
