...
/*************************************************************************/
void Hydro_rnsid(const cGH *cctkGH, CCTK_REAL *x_grid, CCTK_REAL *y_grid,
                 CCTK_REAL *z_grid, CCTK_REAL eos_k,
                 CCTK_REAL eos_ideal_fluid_gamma, CCTK_REAL rnsid_rho_min,
                 CCTK_REAL *Omega_pt, CCTK_REAL *R_e_pt, CCTK_REAL *r_e_pt,
                 CCTK_REAL *mass0_pt, CCTK_REAL *gamma_center)
...
{ /* BEGIN RNSID */
  ...
  */
  ...
  e_center = (eos_k * pow(rho_central, eos_ideal_fluid_gamma) /
                  (eos_ideal_fluid_gamma - 1.0) +
              rho_central);

  /* TABULATED EOS OPTION */
  if (strcmp(eos_type, "tab") == 0) {
    /* --V0-- load_eos( eos_file, log_e_tab, log_p_tab, log_h_tab, log_n0_tab,
     * Gamma_tab, &n_tab ); */
    int n_nearest;
    double n0;
    /* ==================================================== */
    ...
    e_center =
        pow(10.0, interp(log_n0_tab, log_e_tab, n_tab, log10(n0), &n_nearest));
  }
...
#pragma omp parallel for
  ...
  make_center(e_center, log_e_tab, log_p_tab, log_h_tab, n_tab, eos_type, eos_k,
              eos_ideal_fluid_gamma, &p_center, &h_center);
  ...
  if (strcmp(recover_2Dmodel, "yes") == 0 &&
    ...
    guess(s_gp, eos_type, eos_k, e_center, p_center, p_surface, e_surface,
          eos_ideal_fluid_gamma, log_e_tab, log_p_tab, log_h_tab, n_tab,
          rho_potential, gama, alpha, omega, &r_e);
    ...
    */
    ...
    for (r_ratio = 1.0; r_ratio > axes_ratio; r_ratio -= 0.1) {
      ...
      message = (char *)malloc(200 * sizeof(char));
      sprintf(message, " Reducing a_ratio: Iteration for a_ratio = %g",
              r_ratio);
      CCTK_INFO(message);
      free(message);
      iterate(s_gp, mu, eos_type, eos_k, log_e_tab, log_p_tab, log_h_tab, n_tab,
              eos_ideal_fluid_gamma, r_ratio, h_center, enthalpy_min, a_check,
              accuracy, print_dif, cf, &r_e, rho_potential, gama, alpha, omega,
              energy, pressure, enthalpy, velocity_sq, &Omega, rotation_type,
              A_diff, &Omega_e, Omega_diff, RNS_lmax);
    }

    // CCTK_REAL cf = 1.0;
    iterate(s_gp, mu, eos_type, eos_k, log_e_tab, log_p_tab, log_h_tab, n_tab,
            eos_ideal_fluid_gamma, axes_ratio, h_center, enthalpy_min, a_check,
            accuracy, print_dif, cf, &r_e, rho_potential, gama, alpha, omega,
            energy, pressure, enthalpy, velocity_sq, &Omega, rotation_type,
            A_diff, &Omega_e, Omega_diff, RNS_lmax);
  }
  ...
  if (strcmp(save_2Dmodel, "yes") == 0 && recovered_from_2d == 0) {
    ...
    if (CCTK_MyProc(cctkGH) == 0) {
      hdf5_save_var(&sdiv, &mdiv, model2D_file, eos_type, eos_file, &eos_k,
                    &eos_ideal_fluid_gamma, rotation_type, &A_diff, &axes_ratio,
                    &rho0_center, &r_e, s_gp, mu, rho_potential, gama, alpha,
                    omega, energy, pressure, enthalpy, velocity_sq, &Omega,
                    &Omega_e, Omega_diff);
...
#pragma omp for collapse(3) private(i, j, k) private(                          \
        nu_ijk, B_ijk, alpha_ijk, omega_ijk, nu_dr_ijk, B_dr_ijk,              \
            nu_dtheta_ijk, B_dtheta_ijk, alpha_dtheta_ijk, omega_dr_ijk,       \
            omega_dtheta_ijk, rho_0_ijk, energy_ijk, pressure_ijk,             \
            distance_ijk, Omega_ijk, exp_nu_ijk, exp_alpha_ijk, r_bar_ijk,     \
            dr_dx, dr_dy, dr_dz, dtheta_dx, dtheta_dy, dtheta_dz, alpha_dx,    \
            alpha_dy, omega_dx, omega_dy, omega_dz)
  for (i = 1; i <= nx; i++)
    for (j = 1; j <= ny; j++)
      for (k = 1; k <= nz; k++) {
        ...
        grid_interp_all(s_gp, mu, r_e, nx, ny, nz, x_grid, y_grid, z_grid, i, j,
                        k, nu, B, alpha, omega, nu_dr, B_dr, alpha_dr, omega_dr,
        ...
        if (x_i == 0.0 && y_j == 0.0) {
          ...
          gxx[i - 1 + nx * (j - 1 + ny * (k - 1))] =
              (SQ(B_ijk * y_j / exp_nu_ijk) + SQ(exp_alpha_ijk * x_i)) /
              (SQ(x_i) + SQ(y_j));

          gxy[i - 1 + nx * (j - 1 + ny * (k - 1))] =
              (SQ(exp_alpha_ijk) - SQ(B_ijk / exp_nu_ijk)) * x_i * y_j /
              (SQ(x_i) + SQ(y_j));

          gxz[i - 1 + nx * (j - 1 + ny * (k - 1))] = 0.0;

          gyy[i - 1 + nx * (j - 1 + ny * (k - 1))] =
        ...
        velx[i - 1 + nx * (j - 1 + ny * (k - 1))] =
            (omega_ijk - Omega_ijk) * y_j / exp_nu_ijk;

        vely[i - 1 + nx * (j - 1 + ny * (k - 1))] =
        ...
        /* ATMOSPERE SETTINGs                      */
        /* *************************************** */
        if ((rho[i - 1 + nx * (j - 1 + ny * (k - 1))] <
             (1.0 + RNS_atmo_tolerance) * rho_0_atm)) {
