...
/* Load EOS file.                                                        */
/*************************************************************************/
void load_eos(char eos_file[], double log_e_table[MAX_NTAB],
              double log_p_table[MAX_NTAB], double log_h_table[MAX_NTAB],
              double log_n0_table[MAX_NTAB], int *n_tab) {
  int i; /* counter */
  ...
  for (i = 1; i <= (*n_tab); i++) {
    ...
    log_e_table[i] = log10(rho * cactusM *
                           cactusV); /* multiply by C^2 to get energy density */
...
/*C*/
/*******************************************************************/
double e_at_p(double pp, double log_e_tab[MAX_NTAB], double log_p_tab[MAX_NTAB],
              int n_tab, int *n_nearest_pt, char eos_type[], double eos_k,
              double Gamma_P) {
  if (strcmp(eos_type, "tab") == 0)
    return pow(10.0,
               interp(log_p_tab, log_e_tab, n_tab, log10(pp), n_nearest_pt));
  else
...
/*C*/
/*******************************************************************/
double p_at_e(double ee, double log_p_tab[MAX_NTAB], double log_e_tab[MAX_NTAB],
              int n_tab, int *n_nearest_pt) {
  return pow(10.0,
             interp(log_e_tab, log_p_tab, n_tab, log10(ee), n_nearest_pt));
}

/*C*/
/*******************************************************************/
double p_at_h(double hh, double log_p_tab[MAX_NTAB], double log_h_tab[MAX_NTAB],
              int n_tab, int *n_nearest_pt) {
  return pow(10.0,
             interp(log_h_tab, log_p_tab, n_tab, log10(hh), n_nearest_pt));
}

/*C*/
/*******************************************************************/
double h_at_p(double pp, double log_h_tab[MAX_NTAB], double log_p_tab[MAX_NTAB],
              int n_tab, int *n_nearest_pt) {
  return pow(10.0,
             interp(log_p_tab, log_h_tab, n_tab, log10(pp), n_nearest_pt));
}

/*C*/
/*******************************************************************/
double n0_at_e(double ee, double log_n0_tab[MAX_NTAB],
               double log_e_tab[MAX_NTAB], int n_tab, int *n_nearest_pt) {
  return pow(10.0,
             interp(log_e_tab, log_n0_tab, n_tab, log10(ee), n_nearest_pt));
}

/*C*/
/***************************************************************/
void make_center(double e_center, double log_e_tab[MAX_NTAB],
                 double log_p_tab[MAX_NTAB], double log_h_tab[MAX_NTAB],
                 int n_tab, char eos_type[], double eos_k, double Gamma_P,
                 double *p_center, double *h_center) {
  int n_nearest;
...
void comp_values(double s_gp[SDIV + 1], double mu[MDIV + 1], double r_ratio,
                 ...
                 double log_e_tab[MAX_NTAB], double log_n0_tab[MAX_NTAB],
                 int n_tab, double Omega, double **rho, double **gama,
                 double **alpha, double **omega, double **energy,
                 double **pressure, double **enthalpy, double **velocity_sq,
                 double *Mass, double *Mass_0, double *T, double *W,
                 double *Omega_K, double *R_e, char rotation_type[],
                 double **Omega_diff, double *J) {
  int s, m, n_nearest;
  ...
  double **velocity,   /* velocity */
      **rho_0,         /* rest mass density */
      ...
      d_o_e[SDIV + 1], d_g_e[SDIV + 1], d_r_e[SDIV + 1], doe, dge, dre, vek,
      s_e, gama_mu_0[SDIV + 1], rho_mu_0[SDIV + 1], omega_mu_0[SDIV + 1],
      Mass_p, T_diff, D_T[SDIV + 1]; /* int. quantity for J */
    ...
    for (s = 1; s <= SDIV; s++)
      for (m = 1; m <= MDIV; m++) {
        if (energy[s][m] > e_surface)
          /*             rho_0[s][m]=n0_at_e(energy[s][m], log_n0_tab,
             log_e_tab, n_tab, &n_nearest)*MB*KSCALE*SQ(C); */
          /*this conversion factor should be MB*cactusM*/
          rho_0[s][m] =
              n0_at_e(energy[s][m], log_n0_tab, log_e_tab, n_tab, &n_nearest) *
              MB * cactusM;
        else
          rho_0[s][m] = 0.0;
      }
  ...
  for (s = 1; s <= SDIV; s++) {
    ...
    for (m = 1; m <= MDIV - 2; m += 2) {
               ...
               (((energy[s][m] + pressure[s][m]) / (1.0 - velocity_sq[s][m])) *
                    (1.0 + velocity_sq[s][m] +
                     (2.0 * s_gp[s] * sqrt(velocity_sq[s][m]) /
                      (1.0 - s_gp[s])) *
                         sqrt(1.0 - mu[m] * mu[m]) * r_e * omega[s][m] *
                         exp(-rho[s][m])) +
                2.0 * pressure[s][m])
                   ...
                   (1.0 - velocity_sq[s][m + 1])) *
                      (1.0 + velocity_sq[s][m + 1] +
                       (2.0 * s_gp[s] * sqrt(velocity_sq[s][m + 1]) /
                        (1.0 - s_gp[s])) *
                           sqrt(1.0 - mu[m + 1] * mu[m + 1]) * r_e *
                           omega[s][m + 1] * exp(-rho[s][m + 1])) +
                  2.0 * pressure[s][m + 1])

           + exp(2.0 * alpha[s][m + 2] + gama[s][m + 2]) *
                 (((energy[s][m + 2] + pressure[s][m + 2]) /
                   (1.0 - velocity_sq[s][m + 2])) *
                      (1.0 + velocity_sq[s][m + 2] +
                       (2.0 * s_gp[s] * sqrt(velocity_sq[s][m + 2]) /
                        (1.0 - s_gp[s])) *
                           sqrt(1.0 - mu[m + 2] * mu[m + 2]) * r_e *
                           omega[s][m + 2] * exp(-rho[s][m + 2])) +
                  2.0 * pressure[s][m + 2]));

      D_m_0[s] += (1.0 / (3.0 * (MDIV - 1))) *
                  (exp(2.0 * alpha[s][m] + (gama[s][m] - rho[s][m]) / 2.0) *
                       rho_0[s][m] / sqrt(1.0 - velocity_sq[s][m])

                   + 4.0 *
                         exp(2.0 * alpha[s][m + 1] +
                             (gama[s][m + 1] - rho[s][m + 1]) / 2.0) *
                         rho_0[s][m + 1] / sqrt(1.0 - velocity_sq[s][m + 1])

                   + exp(2.0 * alpha[s][m + 2] +
                         (gama[s][m + 2] - rho[s][m + 2]) / 2.0) *
      ...
      D_J[s] +=
          (1.0 / (3.0 * (MDIV - 1))) *
          (sqrt(1.0 - mu[m] * mu[m]) *
               exp(2.0 * alpha[s][m] + gama[s][m] - rho[s][m]) *
      ...
      D_T[s] +=
          ...
          (sqrt(1.0 - mu[m] * mu[m]) *
               exp(2.0 * alpha[s][m] + gama[s][m] - rho[s][m]) *
               (energy[s][m] + pressure[s][m]) * sqrt(velocity_sq[s][m]) *
               Omega_diff[s][m] / (1.0 - velocity_sq[s][m])

           + 4.0 * sqrt(1.0 - mu[m + 1] * mu[m + 1]) *
                 exp(2.0 * alpha[s][m + 1] + gama[s][m + 1] - rho[s][m + 1]) *
                 (energy[s][m + 1] + pressure[s][m + 1]) *
                 sqrt(velocity_sq[s][m + 1]) * Omega_diff[s][m + 1] /
                 (1.0 - velocity_sq[s][m + 1])

           + sqrt(1.0 - mu[m + 2] * mu[m + 2]) *
                 exp(2.0 * alpha[s][m + 2] + gama[s][m + 2] - rho[s][m + 2]) *
                 (energy[s][m + 2] + pressure[s][m + 2]) *
                 sqrt(velocity_sq[s][m + 2]) * Omega_diff[s][m + 2] /
                 (1.0 - velocity_sq[s][m + 2]));
    }
  }
  ...
  for (s = 1; s <= SDIV - 4; s += 2) {
    (*Mass) +=
        (SMAX / (3.0 * (SDIV - 1))) *
        (pow(sqrt(s_gp[s]) / (1.0 - s_gp[s]), 4.0) * D_m[s] +
         4.0 * pow(sqrt(s_gp[s + 1]) / (1.0 - s_gp[s + 1]), 4.0) * D_m[s + 1] +
         pow(sqrt(s_gp[s + 2]) / (1.0 - s_gp[s + 2]), 4.0) * D_m[s + 2]);

    (*Mass_0) +=
        (SMAX / (3.0 * (SDIV - 1))) *
        (pow(sqrt(s_gp[s]) / (1.0 - s_gp[s]), 4.0) * D_m_0[s] +
         4.0 * pow(sqrt(s_gp[s + 1]) / (1.0 - s_gp[s + 1]), 4.0) *
             D_m_0[s + 1] +
         pow(sqrt(s_gp[s + 2]) / (1.0 - s_gp[s + 2]), 4.0) * D_m_0[s + 2]);

    Mass_p +=
        (SMAX / (3.0 * (SDIV - 1))) *
        (pow(sqrt(s_gp[s]) / (1.0 - s_gp[s]), 4.0) * D_m_p[s] +
         4.0 * pow(sqrt(s_gp[s + 1]) / (1.0 - s_gp[s + 1]), 4.0) *
             D_m_p[s + 1] +
    ...
    (*J) +=
        (SMAX / (3.0 * (SDIV - 1))) *
        ((pow(s_gp[s], 3.0) / pow(1.0 - s_gp[s], 5.0)) * D_J[s] +
         4.0 * (pow(s_gp[s + 1], 3.0) / pow(1.0 - s_gp[s + 1], 5.0)) *
             D_J[s + 1] +
         (pow(s_gp[s + 2], 3.0) / pow(1.0 - s_gp[s + 2], 5.0)) * D_J[s + 2]);

    T_diff +=
        (SMAX / (3.0 * (SDIV - 1))) *
        ((pow(s_gp[s], 3.0) / pow(1.0 - s_gp[s], 5.0)) * D_T[s] +
         4.0 * (pow(s_gp[s + 1], 3.0) / pow(1.0 - s_gp[s + 1], 5.0)) *
             D_T[s + 1] +
         (pow(s_gp[s + 2], 3.0) / pow(1.0 - s_gp[s + 2], 5.0)) * D_T[s + 2]);
  }
  ...
  vek = (doe / (8.0 + dge - dre)) * r_e * exp(-rho_equator) +
        sqrt(((dge + dre) / (8.0 + dge - dre)) +
             pow((doe / (8.0 + dge - dre)) * r_e * exp(-rho_equator), 2.0));

  if (strcmp(eos_type, "tab") == 0)
    /*      (*Omega_K) =
     * (C/sqrt(KAPPA))*(omega_equator+vek*exp(rho_equator)/r_e); */ /*conversions*/
...
double dm_dr_is(double r_is, double r, double m, double p, double e_center,
                double p_surface,
                double log_e_tab[MAX_NTAB], /* was [SDIV+1], corrected by
                                               niksterg on 16.10.14 */
                double log_p_tab[MAX_NTAB], /* was [SDIV+1], corrected by
                                               niksterg on 16.10.14 */
                int n_tab, int *n_nearest_pt, char eos_type[], double eos_k,
                double Gamma_P) {
  double dmdr, e_d;
  ...
  if (p < p_surface)
    ...
    e_d = e_at_p(p, log_e_tab, log_p_tab, n_tab, n_nearest_pt, eos_type, eos_k,
                 Gamma_P);

  if (r_is < RMIN)
    dmdr =
        4.0 * PI * e_center * r * r * (1.0 + 4.0 * PI * e_center * r * r / 3.0);
  else
...
double dp_dr_is(double r_is, double r, double m, double p, double e_center,
  ...
  if (r_is < RMIN)
    ...
    dpdr = -(e_d + p) * (m + 4.0 * PI * r * r * r * p) /
           (r * r_is * sqrt(1.0 - 2.0 * m / r));
...
void integrate(int i_check, char eos_type[], double eos_k, double e_center,
               ...
               double Gamma_P, double log_e_tab[MAX_NTAB],
               double log_p_tab[MAX_NTAB], double log_h_tab[MAX_NTAB],
               int n_tab, double r_is_gp[RDIV + 1], double lambda_gp[RDIV + 1],
               double nu_gp[RDIV + 1], double *r_is_final, double *r_final,
               double *m_final) {
  int i = 2, n_nearest;
  ...
  double r,       /* radius */
      ...
      m,          /* mass   */
      nu_s, hh, a1, a2, a3, a4, b1, b2, b3,
      b4, /* coeff. in Runge-Kutta equations */
      c1, c2, c3, c4, k_rescale, r_gp[RDIV + 1], e_d_gp[RDIV + 1];
  ...
  while (p >= p_surface) {
    ...
    e_d = e_at_p(p, log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type, eos_k,
                 Gamma_P);
    ...
    b1 = dm_dr_is(r_is, r, m, p, e_center, p_surface, log_e_tab, log_p_tab,
                  n_tab, &n_nearest, eos_type, eos_k, Gamma_P);
    c1 = dp_dr_is(r_is, r, m, p, e_center, p_surface, log_e_tab, log_p_tab,
                  n_tab, &n_nearest, eos_type, eos_k, Gamma_P);

    a2 = dr_dr_is(r_is + h / 2.0, r + h * a1 / 2.0, m + h * b1 / 2.0);

    b2 = dm_dr_is(r_is + h / 2.0, r + h * a1 / 2.0, m + h * b1 / 2.0,
                  p + h * c1 / 2.0, e_center, p_surface, log_e_tab, log_p_tab,
                  n_tab, &n_nearest, eos_type, eos_k, Gamma_P);

    c2 = dp_dr_is(r_is + h / 2.0, r + h * a1 / 2.0, m + h * b1 / 2.0,
                  p + h * c1 / 2.0, e_center, p_surface, log_e_tab, log_p_tab,
                  n_tab, &n_nearest, eos_type, eos_k, Gamma_P);

    a3 = dr_dr_is(r_is + h / 2.0, r + h * a2 / 2.0, m + h * b2 / 2.0);

    b3 = dm_dr_is(r_is + h / 2.0, r + h * a2 / 2.0, m + h * b2 / 2.0,
                  p + h * c2 / 2.0, e_center, p_surface, log_e_tab, log_p_tab,
                  n_tab, &n_nearest, eos_type, eos_k, Gamma_P);

    c3 = dp_dr_is(r_is + h / 2.0, r + h * a2 / 2.0, m + h * b2 / 2.0,
                  p + h * c2 / 2.0, e_center, p_surface, log_e_tab, log_p_tab,
                  n_tab, &n_nearest, eos_type, eos_k, Gamma_P);

    a4 = dr_dr_is(r_is + h, r + h * a3, m + h * b3);

    b4 = dm_dr_is(r_is + h, r + h * a3, m + h * b3, p + h * c3, e_center,
                  p_surface, log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type,
                  eos_k, Gamma_P);

    c4 = dp_dr_is(r_is + h, r + h * a3, m + h * b3, p + h * c3, e_center,
                  p_surface, log_e_tab, log_p_tab, n_tab, &n_nearest, eos_type,
                  eos_k, Gamma_P);
...
void guess(double s_gp[SDIV + 1], char eos_type[], double eos_k,
           ...
           double Gamma_P, double log_e_tab[MAX_NTAB],
           double log_p_tab[MAX_NTAB], double log_h_tab[MAX_NTAB], int n_tab,
           double **rho, double **gama, double **alpha, double **omega,
           double *r_e) {
  int s, m, n_nearest;

  double r_is_s, r_is_final = 0.0, r_final, m_final, lambda_s, nu_s,
                 r_is_gp[RDIV + 1], lambda_gp[RDIV + 1], nu_gp[RDIV + 1],
                 gama_mu_0[SDIV + 1], rho_mu_0[SDIV + 1], gama_eq, rho_eq,
                 s_e = 0.5;

  integrate(1, eos_type, eos_k, e_center, p_center, p_surface, e_surface,
            Gamma_P, log_e_tab, log_p_tab, log_h_tab, n_tab, r_is_gp, lambda_gp,
            nu_gp, &r_is_final, &r_final, &m_final);

  integrate(2, eos_type, eos_k, e_center, p_center, p_surface, e_surface,
            Gamma_P, log_e_tab, log_p_tab, log_h_tab, n_tab, r_is_gp, lambda_gp,
            nu_gp, &r_is_final, &r_final, &m_final);

  integrate(3, eos_type, eos_k, e_center, p_center, p_surface, e_surface,
            Gamma_P, log_e_tab, log_p_tab, log_h_tab, n_tab, r_is_gp, lambda_gp,
            nu_gp, &r_is_final, &r_final, &m_final);
  ...
  for (s = 1; s <= SDIV; s++) {
    ...
    if (r_is_s < r_is_final) {
      ...
      nu_s = log((1.0 - m_final / (2.0 * r_is_s)) /
                 (1.0 + m_final / (2 * r_is_s)));
    }
  ...
  n_nearest = SDIV / 2;
  gama_eq =
      interp(s_gp, gama_mu_0, SDIV, s_e, &n_nearest);     /* gama at equator */
...
void iterate(double s_gp[SDIV + 1], double mu[MDIV + 1], char eos_type[],
             double eos_k, double log_e_tab[MAX_NTAB],
             double log_p_tab[MAX_NTAB], double log_h_tab[MAX_NTAB], int n_tab,
             double Gamma_P, double r_ratio, double h_center,
             double enthalpy_min, int a_check, double accuracy, int print_dif,
             double cf, double *r_e_new, double **rho, double **gama,
             double **alpha, double **omega, double **energy, double **pressure,
             double **enthalpy, double **velocity_sq, double *Omega,
             char rotation_type[], double A_diff, double *Omega_e,
             double **Omega_diff, int RNS_lmax) {
  int m,           /* counter */
      s,           /* counter */
      n,           /* counter */
      k,           /* counter */
  ...
  double sum_rho = 0.0, /* intermediate sum in eqn for rho */
      ...
      temp2, temp3, temp4, temp5, temp6, temp7, temp8, m1, s1, s2, ea, rsm, gsm,
         omsm, esm, psm, v2sm, mum, sgp, s_1, e_gsm, e_rsm, rho0sm,
         term_in_Omega_h, r_p, s_p, gama_pole_h, /* gama^hat at pole */
      ...
      s_e = 0.5, **da_dm, **dgds, **dgdm, **D1_rho, **D1_gama, **D1_omega,
         **D2_rho, **D2_gama, **D2_omega, **S_gama, **S_rho, **S_omega, **f2n,
         **P_2n, **P1_2n_1, Omega_h, sin_theta[MDIV + 1], theta[MDIV + 1], sk,
         sj, sk1, sj1, r_e, d_e_ds_mu_0[SDIV / 2], s_max, Omega_lower_bracket,
         Omega_upper_bracket, Omega_e_lower_bracket, Omega_e_upper_bracket,
         Omega_c, tol;
    ...
    for (j = 2; j <= SDIV; j++)
      for (n = 1; n <= RNS_lmax; n++)
        for (k = 2; k <= SDIV; k++) {
          ...
          if (k < j) {
            f_rho[j][n + 1][k] =
                f2n[n + 1][j] * sj1 / (sj * f2n[n + 1][k] * sk1 * sk1);
  ...
  do {
    ...
    if (ITERATION_COUNT > MAX_ITERATION) {
      CCTK_VERROR("Max number of iteration %d reached ! Exiting.",
                  MAX_ITERATION);
    }
    ...
    */
    ...
    r_e = sqrt(2 * h_center /
               (gama_pole_h + rho_pole_h - gama_center_h - rho_center_h));
    ...
    if (is_uniform_rotation) {
      ...
      if (r_ratio == 1.0) {
        ...
        if (term_in_Omega_h >= 0.0)
          Omega_h = omega_equator_h +
                    exp(SQ(r_e) * rho_equator_h) * sqrt(term_in_Omega_h);
        else {
          Omega_h = 0.0;
      ...
      (*Omega_e) = zbrent_diff(diff_rotation, r_e, rho_equator_h,
                               ...
                               gama_pole_h, Omega_e_lower_bracket,
                               Omega_e_upper_bracket, tol, A_diff);

      Omega_c = (*Omega_e) + SQ(1.0 / A_diff) * ((*Omega_e) - omega_equator_h) *
                                 exp(-2.0 * SQ(r_e) * rho_equator_h) /
                                 (1.0 - SQ(((*Omega_e) - omega_equator_h) *
                                           exp(-SQ(r_e) * rho_equator_h)));
      ...
      for (s = 2; s <= 2 * SDIV / 3; s++)
        for (m = 1; m <= MDIV - 1; m++) {
          ...
          Omega_diff[s][m] = zbrent_rot(
              rotation_law, r_e, rho[s][m], omega[s][m], s_gp[s], mu[m],
              Omega_c, Omega_lower_bracket, Omega_upper_bracket, tol, A_diff);
        }
    }
      ...
      for (s = 1; s <= SDIV; s++) {
        ...
        for (m = 1; m <= MDIV; m++) {
          ...
          if (r_ratio == 1.0 || s > (SDIV / 2 + 2))
            ...
            velocity_sq[s][m] =
                SQ((Omega_h - omega[s][m]) * (sgp / (1.0 - sgp)) *
      ...
      for (s = 1; s <= SDIV; s++) {
        ...
        for (m = 1; m <= MDIV; m++) {
          ...
          if (r_ratio == 1.0 || s > (SDIV / 2 + 2))
            ...
            velocity_sq[s][m] =
                SQ((Omega_diff[s][m] - omega[s][m]) * (sgp / (1.0 - sgp)) *
                   sin_theta[m] * exp(-rsm * SQ(r_e)));
    ...
    for (s = 1; s <= SDIV; s++) {
      ...
      for (m = 1; m <= MDIV; m++) {
        ...
        if (is_uniform_rotation)
          enthalpy[s][m] =
              enthalpy_min +
              0.5 * (SQ(r_e) * (gama_pole_h + rho_pole_h - gama[s][m] - rsm) -
          ...
          enthalpy[s][m] =
              ...
              0.5 * (SQ(r_e) * (gama_pole_h + rho_pole_h - gama[s][m] - rsm) -
                     log(1.0 - velocity_sq[s][m]) +
                     SQ(A_diff * (Omega_diff[s][m] - Omega_c)));
        ...
        if ((enthalpy[s][m] <= enthalpy_min) || (sgp > s_e)) {
          ...
          if (strcmp(eos_type, "tab") == 0) {
            ...
            pressure[s][m] =
                p_at_h(enthalpy[s][m], log_p_tab, log_h_tab, n_tab, &n_nearest);
            energy[s][m] = e_at_p(pressure[s][m], log_e_tab, log_p_tab, n_tab,
                                  &n_nearest, eos_type, eos_k, Gamma_P);

          } else {
            if (strcmp(eos_type, "poly") == 0) {
              rho0sm = pow(((Gamma_P - 1.0) / (Gamma_P * eos_k)) *
                               (exp(enthalpy[s][m]) - 1.0),
                           1.0 / (Gamma_P - 1.0));
    ...
    if (is_uniform_rotation) {
      ...
      for (s = 1; s <= SDIV; s++)
        for (m = 1; m <= MDIV; m++) {
          ...
          S_rho[s][m] =
              e_gsm * (0.5 * ea * (esm + psm) * s2 * (1.0 + v2sm) / (1.0 - v2sm)
      ...
      for (s = 1; s <= SDIV; s++)
        for (m = 1; m <= MDIV; m++) {
          ...
          S_omega[s][m] =
              e_gsm * e_rsm *
              (-ea * (Omega_diff[s][m] - omsm) * (esm + psm)
    ...
    for (n = 1; n <= RNS_lmax; n++)
      for (k = 1; k <= SDIV; k++) {
        for (m = 1; m <= MDIV - 2; m += 2) {
          ...
          sum_gama +=
              (DM / 3.0) *
              (sin((2.0 * n - 1.0) * theta[m]) * S_gama[k][m] +
               4.0 * sin((2.0 * n - 1.0) * theta[m + 1]) * S_gama[k][m + 1] +
               sin((2.0 * n - 1.0) * theta[m + 2]) * S_gama[k][m + 2]);

          sum_omega +=
              (DM / 3.0) *
              (sin_theta[m] * P1_2n_1[m][n + 1] * S_omega[k][m] +
               4.0 * sin_theta[m + 1] * P1_2n_1[m + 1][n + 1] *
                   S_omega[k][m + 1] +
    ...
    for (s = 1; s <= SDIV; s++) {
      for (k = 1; k <= SDIV - 2; k += 2) {
        sum_rho +=
            (DS / 3.0) * (f_rho[s][n + 1][k] * D1_rho[n + 1][k] +
    ...
    for (s = 1; s <= SDIV; s++)
      for (n = 1; n <= RNS_lmax; n++) {
        for (k = 1; k <= SDIV - 2; k += 2) {
          sum_rho += (DS / 3.0) *
                     (f_rho[s][n + 1][k] * D1_rho[n + 1][k] +
                      4.0 * f_rho[s][n + 1][k + 1] * D1_rho[n + 1][k + 1] +
                      f_rho[s][n + 1][k + 2] * D1_rho[n + 1][k + 2]);

          sum_gama += (DS / 3.0) *
                      (f_gama[s][n + 1][k] * D1_gama[n + 1][k] +
      ...
      for (m = 1; m <= MDIV; m++) {
        ...
        for (n = 1; n <= RNS_lmax; n++) {
          ...
          if (m == MDIV) {
            ...
            sum_omega +=
                -e_rsm * e_gsm *
                (P1_2n_1[m][n + 1] / (2.0 * n * (2.0 * n - 1.0) * temp1)) *
                D2_omega[s][n + 1];

            sum_gama +=
                -(2.0 / PI) * e_gsm *
                (sin((2.0 * n - 1.0) * theta[m]) / ((2.0 * n - 1.0) * temp1)) *
                D2_gama[s][n + 1];
          }
        }
    ...
    if (r_ratio == 1.0) {
      ...
      for (s = 2; s <= SDIV; s++)
        for (m = 1; m <= MDIV; m++) {
          ...
          temp1 = 2.0 * SQ(sgp) * (sgp / (1.0 - sgp)) * m1 * d_omega_s *
                      d_omega_m
          ...
          temp2 =
              1.0 / (m1 * SQ(1.0 + s1 * d_gama_s) + SQ(-mum + m1 * d_gama_m));
          ...
          temp6 = s1 * m1 *
                  (0.5 * (d_rho_s + d_gama_s) * (d_rho_m + d_gama_m)
    ...
    for (s = 1; s <= SDIV; s++) {
      ...
      for (m = 1; m <= MDIV - 1; m++)
        alpha[s][m + 1] =
            alpha[s][m] + 0.5 * DM * (da_dm[s][m + 1] + da_dm[s][m]);
    }
  ...
  if (strcmp(eos_type, "tab") == 0)
    /*      (*Omega) = Omega_h*C/(r_e*sqrt(KAPPA)); */ /*this should be a
                                                          conversion*/
    (*Omega) = Omega_h / r_e;
  else
    (*Omega) = Omega_h / r_e;
