...
/* allocate a double matrix with subscript range m[nrl..nrh][ncl..nch] */
{
  ...
  m[nrl] =
      (double *)malloc((size_t)((nrow * ncol + ARRAY_END) * sizeof(double)));
  if (!m[nrl])
...
double ***tensor_allocate(long nrl, long nrh, long ncl, long nch, long ndl,
                          long ndh)
/* allocate a double 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
  ...
  t[nrl][ncl] = (double *)malloc(
      (size_t)((nrow * ncol * ndep + ARRAY_END) * sizeof(double)));
  if (!t[nrl][ncl])
...
void tensor_free(double ***t, long nrl, long nrh, long ncl, long nch, long ndl,
                 long ndh)
...
/*************************************************************************/
double interp_4(double xp[5], double yp[5], int np, double xb) {
 ...
 *************************************************************************/
void grid_interp(double **old, double *s_gp, double *mu, double r_e, int nx,
                 int ny, int nz,
                 // double *x_grid,
                 // double *y_grid,
                 // double *z_grid,
                 ...
                 // int j,
                 // int k,
                 PointDesc p, double *vnew, int sign) {

  int s, m, s_nearest, m_nearest, /* nearest points in interpolation */
  ...
  if (p.z < 0.0)
    (*vnew) = (1.0 * sign) * interp_4(s_4, old_m, 4, s_c);
  else
    (*vnew) = interp_4(s_4, old_m, 4, s_c);
}

/*************************************************************************
 * Interpolation between two different grids - all
 *************************************************************************/
void grid_interp_all(
    double *s_gp, double *mu, double r_e, int nx, int ny, int nz,
    // double *x_grid,
    // double *y_grid,
    // double *z_grid,
    // int i,
    // int j,
    // int k,
    PointDesc p, double **nu, double **B, double **alpha, double **omega,
    double **nu_dr, double **B_dr, double **alpha_dr, double **omega_dr,
    double **nu_dth, double **B_dth, double **alpha_dth, double **omega_dth,
    double **rho_0, double **energy, double **pressure, double *nu_c,
    double *B_c, double *alpha_c, double *omega_c, double *nu_dr_c,
    double *B_dr_c, double *alpha_dr_c, double *omega_dr_c, double *nu_dth_c,
    double *B_dth_c, double *alpha_dth_c, double *omega_dth_c, double *rho_0_c,
    double *energy_c, double *pressure_c, double *distance_c,
    double **Omega_diff, double *Omega_diff_c) {

  grid_interp(nu, s_gp, mu, r_e, nx, ny, nz, p, nu_c, 1);

  grid_interp(B, s_gp, mu, r_e, nx, ny, nz, p, B_c, 1);

  grid_interp(alpha, s_gp, mu, r_e, nx, ny, nz, p, alpha_c, 1);

  grid_interp(omega, s_gp, mu, r_e, nx, ny, nz, p, omega_c, 1);

  grid_interp(nu_dr, s_gp, mu, r_e, nx, ny, nz, p, nu_dr_c, 1);

  grid_interp(B_dr, s_gp, mu, r_e, nx, ny, nz, p, B_dr_c, 1);

  grid_interp(alpha_dr, s_gp, mu, r_e, nx, ny, nz, p, alpha_dr_c, 1);

  grid_interp(omega_dr, s_gp, mu, r_e, nx, ny, nz, p, omega_dr_c, 1);

  grid_interp(nu_dth, s_gp, mu, r_e, nx, ny, nz, p, nu_dth_c, -1);

  grid_interp(B_dth, s_gp, mu, r_e, nx, ny, nz, p, B_dth_c, -1);

  grid_interp(alpha_dth, s_gp, mu, r_e, nx, ny, nz, p, alpha_dth_c, -1);

  grid_interp(omega_dth, s_gp, mu, r_e, nx, ny, nz, p, omega_dth_c, -1);

  grid_interp(rho_0, s_gp, mu, r_e, nx, ny, nz, p, rho_0_c, 1);

  grid_interp(energy, s_gp, mu, r_e, nx, ny, nz, p, energy_c, 1);

  grid_interp(pressure, s_gp, mu, r_e, nx, ny, nz, p, pressure_c, 1);

  grid_interp(Omega_diff, s_gp, mu, r_e, nx, ny, nz, p, Omega_diff_c, 1);
}
