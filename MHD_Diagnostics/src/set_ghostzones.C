...
extern "C"
void set_ghostzones(CCTK_ARGUMENTS) 
{
...
#pragma omp parallel for
   for (int k = 0; k < cctk_nghostzones[2]; k++)
   {
	 ...
	 const size_t index_copy = CCTK_GFINDEX3D(cctkGH, i, j, cctk_nghostzones[2]);
	...
	kin_acceleration_spatial_x[index]     = kin_acceleration_spatial_x[index_copy];
	kin_acceleration_spatial_y[index]     = kin_acceleration_spatial_y[index_copy];
        kin_acceleration_spatial_z[index]     = kin_acceleration_spatial_z[index_copy];
   ...
   for (int k = cctk_lsh[2]-cctk_nghostzones[2]; k < cctk_lsh[2]; k++)
   {
     for (int j = 0; j < cctk_lsh[1]; j++)
     {
       for (int i = 0; i < cctk_lsh[0]; i++)
       {
         ...
         const size_t index_copy = CCTK_GFINDEX3D(cctkGH, i, j, cctk_lsh[2]-cctk_nghostzones[2]-1);
...
#pragma omp parallel for
   for (int k = 0; k < cctk_lsh[2]; k++)
   {
	 ...
	 const size_t index_copy = CCTK_GFINDEX3D(cctkGH, i, cctk_nghostzones[1], k);
   ...
   for (int k = 0; k < cctk_lsh[2]; k++)
   {
     for (int j = cctk_lsh[1]-cctk_nghostzones[1]; j < cctk_lsh[1]; j++)
     {
       for (int i = 0; i < cctk_lsh[0]; i++)
       {
         ...
         const size_t index_copy = CCTK_GFINDEX3D(cctkGH, i, cctk_lsh[1]-cctk_nghostzones[1]-1, k);
...
#pragma omp parallel for
   for (int k = 0; k < cctk_lsh[2]; k++)
   {
	 ...
	 const size_t index_copy = CCTK_GFINDEX3D(cctkGH, cctk_nghostzones[0], j, k);
   ...
   for (int k = 0; k < cctk_lsh[2]; k++)
   {
     for (int j = 0; j < cctk_lsh[1]; j++)
     {
       for (int i = cctk_lsh[0]-cctk_nghostzones[0]; i < cctk_lsh[0]; i++)
       {
         ...
         const size_t index_copy = CCTK_GFINDEX3D(cctkGH, cctk_lsh[0]-cctk_nghostzones[0]-1, j, k);
