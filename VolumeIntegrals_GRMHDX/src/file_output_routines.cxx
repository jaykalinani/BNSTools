...
CREATE_OUTDIR(CCTK_ARGUMENTS, const char *input_dir, char *&actual_dir) {
  ...
  if (izzle < 0) {
    CCTK_VWarn(1, __LINE__, __FILE__, CCTK_THORNSTRING,
               "VolumeIntegrals_GRMHDX:file_output_routines.C Problem creating "
               "directory '%s' "
               "for output",
               actual_dir);
  } else if (izzle >= 0 && verbose == 1) {
    CCTK_VInfo(CCTK_THORNSTRING, "Outputting to directory '%s'", actual_dir);
  }
}

CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE inline void
OUTDIR_NAME(CCTK_ARGUMENTS, const char *input_dir, char *&actual_dir) {
  DECLARE_CCTK_PARAMETERS;
  const char *_dir = input_dir;
...
extern "C" void VI_GRMHDX_file_output(CCTK_ARGUMENTS) {
  ...
  if (CCTK_MyProc(cctkGH) == 0 && cctk_iteration % VolIntegral_out_every == 0 &&
    ...
    char *filename = (char *)malloc(strlen(actual_dir) +
                                    strlen("volume_integrals-GRMHDX.asc") +
                                    10); // <- Tiny amount of memory.
    sprintf(filename, "%svolume_integrals-GRMHDX.asc", actual_dir);
    FILE *file = fopen(filename, "a+");
    if (!file) {
      CCTK_VWarn(1, __LINE__, __FILE__, CCTK_THORNSTRING,
                 "VolumeIntegrals_GRMHDX: Cannot open output file '%s'",
                 filename);
      exit(1);
    } else {
      ...
      if (size == 0) {
        char *header_buffer =
            (char *)malloc(sizeof(char) * 500000); // <- Tiny amount of memory.
        ...
        for (int which_integral = 1; which_integral <= NumIntegrals;
             which_integral++) {
          if (volintegral_inside_sphere__radius[which_integral] > 0 &&
              volintegral_outside_sphere__radius[which_integral] > 0) {
            sprintf(
                header_buffer,
      ...
      if (enable_time_reparameterization) {
        ...
        //  xx)*Erf((-t0 + xx)/w))/2. Parser: echo
        //  "(((-Power(E,-Power(t0,2)/Power(w,2)) + Power(E,-Power(t0 -
        //  xx,2)/Power(w,2)))*w)/Sqrt(Pi) + xx - t0*Erf(t0/w) + (-t0 +
        //  xx)*Erf((-t0 + xx)/w))/2."|sed
        //  "s/Power(E,/exp(/g;s/Power(/pow(/g;s/Sqrt(/sqrt(/g;s/Erf(/erf(/g;s/Pi/M_PI/g"
        //  Parser output: (((-exp(-pow(t0,2)/pow(w,2)) + exp(-pow(t0 -
        //  xx,2)/pow(w,2)))*w)/sqrt(M_PI) + xx - t0*erf(t0/w) + (-t0 +
        //  xx)*erf((-t0 + xx)/w))/2.
        double t0 = VIv_time_reparam_t0;
        double w = VIv_time_reparam_w;
        *physical_time =
            (((-exp(-pow(t0, 2) / pow(w, 2)) +
               exp(-pow(t0 - xx, 2) / pow(w, 2))) *
              w) /
                 sqrt(M_PI) +
             xx - t0 * erf(t0 / w) + (-t0 + xx) * erf((-t0 + xx) / w)) /
            2.;
        // 0.5 * ( (xx-t0)*erf((xx-t0)/w) - t0*erf(2.0) +
        // (w/sqrt(M_PI))*(exp(-(xx-t0)*(xx-t0)/(w*w)) - 1.0/exp(t0*t0/(w*w))) +
        // xx);
      ...
      for (int which_integral = 1; which_integral <= NumIntegrals;
        ...
        for (int reduction = 0; reduction < num_reductions; reduction++) {
          if (which_integral == NumIntegrals &&
              reduction == num_reductions - 1) {
            sprintf(buffer, "%s%.15e\n", buffer,
                    VolIntegral[4 * (which_integral) + reduction]);
          } else {
            sprintf(buffer, "%s%.15e ", buffer,
                    VolIntegral[4 * (which_integral) + reduction]);
          }
        }
      }
