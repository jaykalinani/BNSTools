...
CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE inline CCTK_REAL compute_sqrtgamma(
...
CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE inline void mean_density_weighted_B(
    const Loop::GF3D2<double> VolIntegrand1, const Loop::GF3D2<double> VolIntegrand2,
    const Loop::PointDesc &p,
    const Loop::GF3D2<const double> Bvecx, const Loop::GF3D2<const double> Bvecy,
    const Loop::GF3D2<const double> Bvecz, const Loop::GF3D2<const double> w_lorentz,
...
CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE inline void volume_norm_B_12(
    const Loop::GF3D2<double> VolIntegrand1, const Loop::GF3D2<double> VolIntegrand2,
    const Loop::GF3D2<double> VolIntegrand3, const Loop::GF3D2<double> VolIntegrand4,
    const Loop::PointDesc &p,
    const Loop::GF3D2<const double> rho0, const CCTK_REAL dens_1, const CCTK_REAL dens_2,
    const Loop::GF3D2<const double> Bvecx, const Loop::GF3D2<const double> Bvecy,
...
CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE inline void kinetic_shibata(
    ...
    const Loop::GF3D2<double> VolIntegrand3,
    const Loop::PointDesc &p,
    const Loop::GF3D2<const double> velxGF, const Loop::GF3D2<const double> velyGF, 
    const Loop::GF3D2<const double> velzGF, const Loop::GF3D2<const double> w_lorentzGF,
    const Loop::GF3D2<const double> rho0, const Loop::GF3D2<const double> epsGF, 
    const Loop::GF3D2<const double> pressGF, const Loop::GF3D2<const double> alpGF,
    const Loop::GF3D2<const double> betaxGF, const Loop::GF3D2<const double> betayGF,
...
// This is just the total kinetic energy
  ...
  VolIntegrand1(p.I) = 0.5*sqrtgamma*(my_rho*(1.0+my_eps)+my_press)*w_lorentz_limited*w_lorentz_limited*(my_lapse*v_2-shift_v);
...
// This is the rotational kinetic energy wrt to the z-axis
  ...
  VolIntegrand2(p.I) = 0.5*sqrtgamma*(my_rho*(1.0+my_eps)+my_press)*w_lorentz_limited*w_lorentz_limited*(my_lapse*v_phi2-v_phi_shift_phi);

// Kinetic energy in z-direction

  VolIntegrand3(p.I) = 0.5*sqrtgamma*(my_rho*(1.0+my_eps)+my_press)*w_lorentz_limited*w_lorentz_limited*(my_lapse*vz_2-shiftz_vz);

}

/* Kinetic energy as given by https://arxiv.org/pdf/2112.08413.pdf */
CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE inline void kinetic_palenzuela(
    const Loop::GF3D2<double> VolIntegrand1, const Loop::GF3D2<double> VolIntegrand2,
    const Loop::GF3D2<double> VolIntegrand3,
    const Loop::PointDesc &p,
    const Loop::GF3D2<const double> velxGF, const Loop::GF3D2<const double> velyGF, 
...
// This is just the total kinetic energy
  ...
  VolIntegrand1(p.I) = 0.5*sqrtgamma*(my_rho*(1.0+my_eps)+my_press)*w_lorentz_limited*w_lorentz_limited*(v_2-shift_v/my_lapse);
...
// This is the rotational kinetic energy wrt to the z-axis
  ...
  VolIntegrand2(p.I) = 0.5*sqrtgamma*(my_rho*(1.0+my_eps)+my_press)*w_lorentz_limited*w_lorentz_limited*(v_phi2-v_phi_shift_phi/my_lapse);

// Kinetic energy in z-direction

  VolIntegrand3(p.I) = 0.5*sqrtgamma*(my_rho*(1.0+my_eps)+my_press)*w_lorentz_limited*w_lorentz_limited*(vz_2-shiftz_vz/my_lapse);
...
// This is just the total kinetic energy
  ...
  VolIntegrand1(p.I) = 0.5*sqrtgamma*(my_rho*(1.0+my_eps)+my_press)*(w_lorentz_limited*w_lorentz_limited*v_2);
...
// This is the rotational kinetic energy wrt to the z-axis
  ...
  VolIntegrand2(p.I) = 0.5*sqrtgamma*(my_rho*(1.0+my_eps)+my_press)*w_lorentz_limited*w_lorentz_limited*v_phi2;

// Kinetic energy in z-direction

  VolIntegrand3(p.I) = 0.5*sqrtgamma*(my_rho*(1.0+my_eps)+my_press)*(w_lorentz_limited*w_lorentz_limited*vz_2);
...
// Covariant 3 velocity
  ...
  VolIntegrand1(p.I) = sqrtgamma*( my_press*(w_lorentz_limited*w_lorentz_limited*v_2) + 
...
CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE inline void thermal(
    ...
    const Loop::GF3D2<const double> rho0GF, const Loop::GF3D2<const double> epsGF, 
  ...
  const CCTK_REAL entropy = entropyGF(p.I); // this is supposed to be the entropy per baryon
  ...
  // Total entropy
  VolIntegrand3(p.I) = sqrtgamma*rho0*entropy*w_lorentz_limited/my_baryon_mass;
}
...
// Finally, compute E_r*E^r*sqrtgamma and B_r*B_r*sqrtgamma
  ...
  VolIntegrand2(p.I) = (E_2+sqrtgammaE_cov[2]*sqrtgammaE_contra[2]/sqrtgamma
