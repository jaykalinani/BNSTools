...
/* Volume averaged norm of B in +/- z domain */
CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE inline void volume_norm_B_zsplit(
    const Loop::GF3D2<double> VolIntegrand1,
    const Loop::GF3D2<double> VolIntegrand2,
    const Loop::GF3D2<double> VolIntegrand3,
    const Loop::GF3D2<double> VolIntegrand4, const Loop::PointDesc &p,
    ...
    const Loop::GF3D2<const double> gyy, const Loop::GF3D2<const double> gyz,
    const Loop::GF3D2<const double> gzz, const double cms_x,
    const double cms_y, const double cms_z, const int which_z) {

  if (which_z * (p.z - cms_z) > 0.0) {
    const CCTK_REAL gammaDD00 = gxx(p.I);
    const CCTK_REAL gammaDD01 = gxy(p.I);
    const CCTK_REAL gammaDD02 = gxz(p.I);
    const CCTK_REAL gammaDD11 = gyy(p.I);
...
/* Volume averaged mth mode of toroidal, poloidal B in region (dens_1,dens_2]: */
CCTK_DEVICE CCTK_HOST CCTK_ATTRIBUTE_ALWAYS_INLINE inline void volume_norm_B_m_12(
    const Loop::GF3D2<double> VolIntegrand1,
    const Loop::GF3D2<double> VolIntegrand2,
    const Loop::GF3D2<double> VolIntegrand3,
    const Loop::GF3D2<double> VolIntegrand4, const Loop::PointDesc &p,
    ...
    const Loop::GF3D2<const double> gyy, const Loop::GF3D2<const double> gyz,
    const Loop::GF3D2<const double> gzz, const double cms_x,
    const double cms_y, const int mm) {
  ...
  if (my_rho > dens_1 && my_rho <= dens_2 && rcyl > 0.1) {
    ...
    VolIntegrand1(p.I) = sqrtgamma * sqrt(abs(B_2_tor)) * sin(mm * atan2(posy, posx));
    VolIntegrand2(p.I) = sqrtgamma * sqrt(abs(B_2_tor)) * cos(mm * atan2(posy, posx));;
    VolIntegrand3(p.I) = sqrtgamma * sqrt(abs(B_2_pol)) * sin(mm * atan2(posy, posx));
    VolIntegrand4(p.I) = sqrtgamma * sqrt(abs(B_2_pol)) * cos(mm * atan2(posy, posx));;
