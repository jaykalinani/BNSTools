	...
	grid.loop_all<1, 1, 1>(
      ...
      z_k = p.z;
		...
		if(x_i==0.0 && y_j==0.0) {
			...
			gxx_cc(p.I) = ( SQ(B_ijk*y_j/exp_nu_ijk)
						+SQ(exp_alpha_ijk*x_i) ) /
						(SQ(x_i)+SQ(y_j));

			gxy_cc(p.I) = ( SQ(exp_alpha_ijk)
					 -SQ(B_ijk/exp_nu_ijk) ) *
					 x_i*y_j/(SQ(x_i)+SQ(y_j));

			gxz_cc(p.I) = 0.0;

			gyy_cc(p.I) = ( SQ(B_ijk*x_i/exp_nu_ijk)
			...
			kxx_cc(p.I) = (  ( SQ(r_bar_ijk)*y_j*omega_dx+
			...
			kxz_cc(p.I) = 0.5*SQ(B_ijk)*y_j*omega_dz/
																	 ( SQ(exp_nu_ijk)*exp_nu_ijk );

			kyy_cc(p.I) = ( ( -SQ(r_bar_ijk)*x_i*omega_dy+
			...
			kyz_cc(p.I) = -0.5*SQ(B_ijk)*x_i*omega_dz/
																	 ( SQ(exp_nu_ijk)*exp_nu_ijk );

			kzz_cc(p.I) = (y_j*alpha_dx-x_i*alpha_dy)*
		...
		velx(p.I) = (omega_ijk-Omega_ijk)
																		*y_j/exp_nu_ijk;

		vely(p.I) = -(omega_ijk-Omega_ijk)
																		*x_i/exp_nu_ijk;
