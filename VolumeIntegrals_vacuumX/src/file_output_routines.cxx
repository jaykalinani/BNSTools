    ...
    if (izzle < 0)
      {
        CCTK_VWarn (1, __LINE__, __FILE__, CCTK_THORNSTRING,
                    "VolumeIntegrals_vacuumX:file_output_routines.cxx Problem creating directory '%s' "
...
extern "C" void VI_vacuumX_file_output_routine_Startup(CCTK_ARGUMENTS) {
  DECLARE_CCTK_PARAMETERS;
...
extern "C" void VI_vacuumX_file_output(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  *physical_time = cctk_time;
  if(CCTK_MyProc(cctkGH)==0 && VolIntegral_out_every > 0 && cctk_iteration%VolIntegral_out_every==0 && enable_file_output==1) {
    char *actual_dir;
    ...
    char *filename = (char *)malloc (strlen (actual_dir) +
				     strlen ("volume_integrals-vacuumX.asc") + 10); // <- Tiny amount of memory.
	...
	sprintf(header_buffer,"# Col. 1: Time (cctk_time)\n");
        ...
        if(enable_time_reparameterization) { sprintf(header_buffer + strlen(header_buffer),"# Col. 2: Physical Time (physical_time)\n"); which_col++; }
	...
	for(int which_integral=1;which_integral<=NumIntegrals;which_integral++) {
	  if(volintegral_inside_sphere__radius[which_integral]>0 && volintegral_outside_sphere__radius[which_integral]>0) {
	    sprintf(header_buffer + strlen(header_buffer),
                    "# Col. %d: %s. Init. IN sphere @ (%e,%e,%e), r=%e. OUT sphere @ (%e,%e,%e), r=%e. Moves/Tracks AMR Centre %d/%d\n",
	    ...
	    sprintf(header_buffer + strlen(header_buffer),
		    "# Col. %d: %s. Init. IN sphere @ (%e,%e,%e), r=%e. Moves/Tracks AMR Centre %d/%d\n",
		    which_col,Integration_quantity_keyword[which_integral],
		    volintegral_sphere__center_x_initial[which_integral],
		    volintegral_sphere__center_y_initial[which_integral],
		    volintegral_sphere__center_z_initial[which_integral],
	    ...
	    sprintf(header_buffer + strlen(header_buffer),
		    "# Col. %d: %s. (Integral over full grid)\n",
		    which_col,Integration_quantity_keyword[which_integral]);
	  }
	    which_col+=VI_vacuumX_number_of_reductions(which_integral);
	}
	fprintf(file,"%s",header_buffer);
      ...
      if(enable_time_reparameterization) {
        ...
        *physical_time = (((-exp(-pow(t0,2)/pow(w,2)) + exp(-pow(t0 - xx,2)/pow(w,2)))*w)/sqrt(M_PI) + xx - t0*erf(t0/w) + (-t0 + xx)*erf((-t0 + xx)/w))/2.;
        ...
        sprintf(buffer + strlen(buffer),"%e ",*physical_time);
      }
      for(int which_integral=1;which_integral<=NumIntegrals;which_integral++) {
	int num_reductions = VI_vacuumX_number_of_reductions(which_integral);
	for(int reduction=0;reduction<num_reductions;reduction++) {
          if(which_integral==NumIntegrals && reduction==num_reductions-1) {
            sprintf(buffer + strlen(buffer), "%.15e\n",VolIntegral[4*(which_integral) + reduction]);
          } else {
            sprintf(buffer + strlen(buffer), "%.15e ", VolIntegral[4*(which_integral) + reduction]);
          }
	}
      }
